//
//  UIGestureRecognizerTrack.swift
//  testAOPDemo
//
//  Created by 管浩 on 2020/5/21.
//  Copyright © 2020 管浩. All rights reserved.
//

import UIKit

class UITapGestureRecognizerTrack: NSObject {
    
    // MARK: 单例
    
    private static var instance: UITapGestureRecognizerTrack = {
        let track = UITapGestureRecognizerTrack.init()
        return track
    }()
    
    open class var shared: UITapGestureRecognizerTrack {
        get {
            return instance
        }
    }
    
    override class func copy() -> Any {
        return instance
    }
    
    override func mutableCopy() -> Any {
        return UITapGestureRecognizerTrack.instance
    }
    
    override init() {
        super.init()
    }
    
    // MARK: 属性
    
    private var recordParams: [String : [String : Any?]] = [:]
    
    // MARK: 拦截的方法
    
    func trackGRAction(_ gesture: UITapGestureRecognizer, action: Selector, target: UIResponder) -> Void {
        TrackingDataAnalysisHelper.analysisUITapGestureRecognizerTrackingData(ofGesture: gesture, action: action, target: target)
    }
}
